...
const AddPlayerForm = () => {
  ...
  const addPlayerToData = async () => {
    ...
    if (data !== undefined && data !== null) {
      if (firstName === '' || lastName === '' || shirtNumber === '' || position === '' || age === '' || height === '') {
        Toast.show({
          position: 'bottom',
          type: 'error',
          text1: 'Warning',
      ...
      let res = await userHandler.AddPlayerById(data.userId, firstName, lastName, shirtNumber, position, age, height)
    ...
    <View style={styles.centeredView}>
      ...
      <View style={styles.FormView}>
        <View style={styles.TextInput}>
          ...
          <TextInput style={styles.input} onChangeText={text => setShirtNumber(text)} placeholder="Shirt Number" />
        </View>

        <View style={styles.TextInput2}>
          <TextInput style={styles.input} onChangeText={text => setPosition(text)} placeholder="Position" />
          <TextInput style={styles.input} onChangeText={text => setAge(text)} placeholder="Age" />
          <TextInput style={styles.input} onChangeText={text => setHeight(text)} placeholder="Height" />
        </View>
        <View style={{ marginLeft: 20 }}>
          <TouchableOpacity style={styles.AddPlayerButton} onPress={addPlayerToData}>
